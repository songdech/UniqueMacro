...
{
    public partial class Form_DashboardRE_Urgent : Form
    {
        ...
        private static string Version = "Result Routine";
        ...
        public Form_DashboardRE_Urgent()
        {
            InitializeComponent();
        ...
        private void Form_DashboardRE_Urgent_Load(object sender, EventArgs e)
        {
            this.Text = Version;
            try
            {
        ...
        private void CreateColums_REQUESTS_RE_Routine(TreeList tl, DataSet ds)
        {
            //  CreateColumns
            // 0 = Patnumber
            // 1 = Accessnumber 
        ...
        private void CreateColums_MB_REQUESTS_RE_Routine(TreeList tl, DataSet ds)
        {
        ...
        private void SearchData_REQUEST_Status()
        {
            ...
            sql.Append("SELECT DISTINCT" +
                ...
                " WHERE REQUESTS.URGENT = '1'" +
                " AND (MB_REQUESTS.VALIDATIONSTATUS = '4' OR MB_REQUESTS.VALIDATIONSTATUS is null)" );
            ...
            try
            {
                ...
                ds.Clear();
                adp.Fill(ds, "result_RE");
                SqlDataReader reader = sqlCmd.ExecuteReader();
                CreateColums_REQUESTS_RE_Routine(treeList1, ds);

                treeList1.BeginUnboundLoad();
                treeList1.ClearNodes();
                TreeListNode parentForRootNodes = null;
                if (ds.Tables["result_RE"].Rows.Count > 0)
                {
                    //  CreateColumns
                    // 0 = Patnumber
                    // 1 = Accessnumber 
                    ...
                    for (int i = 0; i < ds.Tables["result_RE"].Rows.Count; i++)
                    {
                        childNode = treeList1.AppendNode(new object[] {
                        ds.Tables["result_RE"].Rows[i]["PATNUMBER"].ToString(),
                        ds.Tables["result_RE"].Rows[i]["ACCESSNUMBER"].ToString(),
                        ds.Tables["result_RE"].Rows[i]["PATID"].ToString(),
                        ds.Tables["result_RE"].Rows[i]["NAME"].ToString(),
                        ds.Tables["result_RE"].Rows[i]["LASTNAME"].ToString(),
                        ds.Tables["result_RE"].Rows[i]["REQDATE"].ToString(),
                        ds.Tables["result_RE"].Rows[i]["COLLECTIONDATE"].ToString()
        ...
        {   // '" + StrcellACCESS + "'
            ...
            try
            {
                ...
                SqlDataReader reader = sqlCmd.ExecuteReader();
                CreateColums_MB_REQUESTS_RE_Routine(treeList2, ds);
