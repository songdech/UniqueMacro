...
{
    public partial class Form_DashboardSR_Routine : Form
    {
        ...
        public Form_DashboardSR_Routine()
        {
            InitializeComponent();
        }
        ...
        private void CreateColums_REQUESTS_SR_Routine(TreeList tl, DataSet ds)
        {
            //  CreateColumns
            // 0 = Patnumber
            // 1 = Accessnumber 
        ...
        private void CreateColums_REQ_TESTS_SR_Routine(TreeList tl, DataSet ds)
        {
        ...
        private void SearchData_REQUEST_Status()
        {
            ...
            try
            {
                ...
                ds.Clear();
                adp.Fill(ds, "result_SR");
                SqlDataReader reader = sqlCmd.ExecuteReader();
                CreateColums_REQUESTS_SR_Routine(treeList1, ds);

                treeList1.BeginUnboundLoad();
                treeList1.ClearNodes();
                TreeListNode parentForRootNodes = null;
                if (ds.Tables["result_SR"].Rows.Count > 0)
                {
                    //  CreateColumns
                    // 0 = Patnumber
                    // 1 = Accessnumber 
                    ...
                    for (int i = 0; i < ds.Tables["result_SR"].Rows.Count; i++)
                    {
                        childNode = treeList1.AppendNode(new object[] {
                        ds.Tables["result_SR"].Rows[i]["PATNUMBER"].ToString(),
                        ds.Tables["result_SR"].Rows[i]["SP_ACCESSNUMBER"].ToString(),
                        ds.Tables["result_SR"].Rows[i]["PATID"].ToString(),
                        ds.Tables["result_SR"].Rows[i]["NAME"].ToString(),
                        ds.Tables["result_SR"].Rows[i]["LASTNAME"].ToString(),
                        ds.Tables["result_SR"].Rows[i]["REQDATE"].ToString(),
                        ds.Tables["result_SR"].Rows[i]["COLLECTIONDATE"].ToString()
        ...
        private void SearchData_REQ_TESTS_Status(string StrcellACCESS)
        {
            DBFactory dbFactory = new DBFactory();
            dbFactory.SetDBConnString(Constr);
            SqlConnection objConn = dbFactory.GetDBConnection();
            ...
            try
            {
                ...
                SqlDataReader reader = sqlCmd.ExecuteReader();
                CreateColums_REQ_TESTS_SR_Routine(treeList2, ds);
        ...
        private void treeList1_Click(object sender, EventArgs e)
        {
            if (treeList1.AllNodesCount > 0)
            {
                ...
                //SearchData_REQ_TESTS_Status(StrcellACCESS);
            }
        }
