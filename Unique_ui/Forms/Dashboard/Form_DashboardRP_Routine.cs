...
{
    public partial class Form_DashboardRP_Routine : Form
    {
        System.Globalization.CultureInfo cultureinfo_ENG = new System.Globalization.CultureInfo("en-US");

        private static string Version = "Report Result";
        ...
        public Form_DashboardRP_Routine()
        {
            InitializeComponent();
        ...
        private void Form_DashboardRP_Routine_Load(object sender, EventArgs e)
        {
            this.Text = Version;
...
OR MB_REQUESTS.VALIDATIONSTATUS= '2') ORDER BY REQDATE ASC";
...
FROM MB_REQUESTS
LEFT OUTER JOIN MB_ACTIONS ON MB_ACTIONS.SUBREQUESTID = MB_REQUESTS.MBREQUESTID
LEFT OUTER JOIN REQUESTS ON MB_REQUESTS.REQUESTID = REQUESTS.REQUESTID
...
WHERE REQUESTS.ACCESSNUMBER = '" + StrcellACCESS + "' AND (MB_ACTIONS.ACTIONTYPE = 2 OR MB_ACTIONS.ACTIONTYPE = 1) ORDER BY MBACTIONID ASC";
        ...
        private void treeList2_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                ...
                if (info.Node != null)
                {
                    treeList2.FocusedNode = info.Node;
                    if (treeList2.Nodes.Count > 0)
                    {
                        this.contextMenuStrip.Show(this.treeList2, e.Location);
        ...
        private void PrintoutMenuItem1_Click(object sender, EventArgs e)
        {
            try
            {
                string Str_Protocolnum = treeList2.FocusedNode.GetDisplayText(0);
                string Str_Accessnumber = treeList1.FocusedNode.GetDisplayText(1);

                Micro_Printout FM = new Micro_Printout(Str_Accessnumber, Str_Protocolnum);
                FM.ShowDialog();
